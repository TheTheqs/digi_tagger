...
class TagTypeLayer(QWidget):
    def __init__(self, tag_type_name: str, tag_names: list[str], on_add_tag_callback):
        super().__init__()
        ...
        self.label = QLabel(tag_type_name)
        layout.addWidget(self.label)

        self.dropdown = Dropdown(tag_names)
        layout.addWidget(self.dropdown)

        self.add_button = ToolButton("ADD", "Adicionar novo Tag", self._on_add_tag)
        layout.addWidget(self.add_button)

        self.setLayout(layout)

    def _on_add_tag(self):
        self.on_add_tag_callback(self.tag_type_name)
